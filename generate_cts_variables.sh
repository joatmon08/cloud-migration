...
cat << EOF > consul_terraform_sync/datacenter.module.tfvars
...
sed -i.bak "s/CONSUL_HTTP_ADDR/${CONSUL_HTTP_ADDR}/g" consul_terraform_sync/config.local.hcl
sed -i.bak 's:PWD:'`pwd`':' consul_terraform_sync/config.local.hcl